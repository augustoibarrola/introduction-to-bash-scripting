...
Run the $(ColorGreen 'cat') command without any command-line 
arguments. It may seem that the process has paused but in fact 
it's $(ColorGreen 'cat') asking for $(ColorBlue 'STDIN'), or "Standard Input". 
$(ColorGreen 'cat') is a simple program and will print the text passed to $(ColorBlue 'STDIN').

However, you can extend the use case by redirecting the input 
to the commands that take $(ColorBlue 'STDIN').

EOF

# ??? What commands exactly take STDIN?  